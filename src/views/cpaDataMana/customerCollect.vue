    ...
    export default {
        data() {
            return {
                ...
                checkedCities: [ '客户名称','账目周期', '抵扣余额','抵扣后余额', '支付状态', '备注'],
                cities: [ '对账日期', '账目周期', '客户名称', '结算金额','核减金额', '抵扣余额','抵扣后余额', '支付状态', '备注'],
                citiesFilterVal:['date','customerName','productName','productChannelCode','channelName','channelProductCode','cooperateMode','settlementAct','price','revenue','amount',],
                citiesFilter:['日期','客户名称','产品名称','上游渠道号','渠道名称','产品包渠道代码','合作模式','业务数据','单价','收入金额（元）','结算状态',],
                table_column: [
                    ...
                    {fixed: false, prop: 'startDate', label: '账目周期', sortable: false, visible: true,
                    ...
                    {fixed: false, prop: 'val_1', label: '抵扣余额 ', sortable: false, visible: true},
                ...
                table_column_bill_detail: [
                    ...
                    {fixed: false, prop: 'settlementAct', label: '业务数据', sortable: false, visible: true},
                    ...
                    {fixed: false, prop: 'amount', label: '结算状态', sortable: true, visible: true},
                ],
...
//                        console.log('');
            ...
            pageInfo() {
                ...
                this.$http({
                    method: 'get',
                    url: this.apiUrl.customerBillList,
                    params: params
                }).then(res => {
                    if (res.status = 200) {
...
//                        console.log('');
            ...
            deleteBill(id) {
                this.$confirm('此操作将永久删除该账单, 是否继续?', '提示', {
                    ...
                    this.$http({
                        method: 'delete',
                        url: this.apiUrl.customerBillDel,
                        params: {id: id}
                    }).then(res => {
                        if (res.status == 200) {
                            this.$message({
