            ...
            <div class="info_ad_search">
                <div class="info_a_s_content">
                    <div class="customerProCitySearch">
                        <el-input v-model="contractNo" class="page_d_select" placeholder="请输入合同编号" @keyup.enter.native="pageInfo"></el-input><!--
					  -->
    ...
    export default {
        data() {
            return {
                ...
                checkedCities: ['序号', '合同编号', '客户主体', '开始时间', '结束时间', '推广模式', '合同状态', '审核状态'],
                cities: ['序号', '合同编号', '客户主体', '开始时间', '结束时间', '推广模式', '合同状态', '审核状态'],
                table_column: [
                    {fixed: true, prop: 'id', label: '序号', sortable: true, visible: true},
                    {fixed: false, prop: 'contractNo', label: '合同编号', sortable: false, visible: true},
                    {fixed: false, prop: 'customerName', label: '客户主体', sortable: false, visible: true},
...
//                    console.log(res);
                    if (res.status == 200) {
                        var data = res.data;
                        this.customer_items.options = this.selectPush(data.items,'customerCode','name');
                    } else {
//                        console.log('');
                    }
                }).catch(error => {
...
//                        console.log('');
            ...
            newContract() {
                this.$router.push({path: 'customerContractNew', query: {customerContractId: ''}});
            },
            bodyEvent() {
