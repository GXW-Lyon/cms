...
<script>
    export default {
        ...
        created() {
            this.ziliaoInfo(this.$route.query.id);
        },
        methods: {
            /*获取详情数据*/
            ziliaoInfo(id){
                this.$http({
                    ...
                    if (res.status = 200) {
                        ...
                        this.dateValue=this.customerInfo.cardExpired;
                    }
                }).catch(err => {
                });
            },
            onSubmit() {
                ...
                this.$http({
                    method: 'post',
                    url: this.apiUrl.fdpCusUpdateCustomer,
