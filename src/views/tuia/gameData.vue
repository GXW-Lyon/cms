...
export default {
    data () {
        return {
            ...
            checkedCities: ['日期', '游戏', '渠道', '游戏展现', '游戏展现率', '游戏点击', '游戏点击率'],
            cities: ['日期', '游戏', '渠道', '游戏展现', '游戏展现率', '游戏点击', '游戏点击率'],
            citiesFilterVal: ['date', 'gameName', 'originName', 'gameViewVolume', 'gameViewRate', 'gameClickVolume', 'gameClickRate'],
            table_column: [
                { fixed: true, prop: 'date', label: '日期', sortable: true, visible: true },
                { fixed: false, prop: 'gameName', label: '游戏', sortable: false, visible: true },
    ...
    methods: {
        ...
        pageInfo () {
            ...
            this.$http({
                method: 'post',
                url: this.apiUrl.tuiaAdminLogStatGameData,
                data: {
                    adNo: "",
                ...
                if (res.status == 200) {
                    ...
                    for (var key in data) {
                        for (var index in this.contract_items.options) {
                            if (data[key].gameNo == this.contract_items.options[index].gameNo) {
                                data[key].gameName = this.contract_items.options[index].name;
                            }
                        }
                    }
                    this.tableData = data;
        ...
        importData () {
            ...
            let fileName = [this.params.date, this.customer_items.model_value, this.contract_items.model_value, '游戏数据'].join('_');
